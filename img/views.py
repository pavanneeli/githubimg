...
def index(request):
    if request.method=='POST':
        ...
        r=requests.get(url)
        soup=bs(r.content)     #gets all the HTML code
        profile=soup.find('img',class_='avatar avatar-user width-full border color-bg-primary')['src']
        print(profile)
        new_github=Github(githubuser = github_user ,imagelink = profile  ,username = user)
        new_github.save()
